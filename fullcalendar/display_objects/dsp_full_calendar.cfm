...
<!--- Start Full Calendar --->
<cfset last=arguments.iterator.getLastRecordOnPageIndex()>
<script type='text/javascript'>
			$(document).ready(function() {
				...
				$('#svCalendar').fullCalendar({
					...
					editable: true,
					firstHour: 9,
					eventBackgroundColor: '#99cf53',
					eventBorderColor: '#77b921',
					eventTextColor: '#777',
					events: [
				<cfloop condition="arguments.iterator.hasNext()">
					...
					<cfoutput>
					<cfswitch expression="#arguments.item.getValue('eventColor')#">
						<cfcase value="Blue">
							<cfset backgroundColor= "##d9e9f5">
							<cfset borderColor= "##6b9bbc">
						</cfcase>
						<cfcase value="Green">
							<cfset backgroundColor = "##eaf5d9">
							<cfset borderColor = "##b4dd57">
						</cfcase>
						<cfcase value="Red">
							<cfset backgroundColor = "##f2cdcd">
							<cfset borderColor = "##e68585">
						</cfcase>
						<cfcase value="Orange">
							<cfset backgroundColor = "##ffe8c7">
							<cfset borderColor = "##ffae21">
						</cfcase>
						<cfcase value="Yellow">
							<cfset backgroundColor = "##fffec7">
							<cfset borderColor = "##f8f668">
						</cfcase>
					</cfswitch>
					<cfset record=arguments.iterator.getRecordIndex()>
						{
							title: '#JSStringFormat(arguments.item.getTitle())#',
							start: '#arguments.item.getValue('displayStart')#',
							end: '#arguments.item.getValue('displayStop')#',
							url: '#arguments.item.getURL()#?date=#dateFormat(arguments.item.getValue('displayStart'), "mm-dd-yyyy")#',
							backgroundColor: '#backgroundColor#',
							borderColor: '#borderColor#',
							<cfif arguments.item.getValue('displayStart') contains "00:00:00">
							<cfelseif arguments.item.getValue('displayStop') contains "23:59:00">
							allDay: true,
							<cfelse>
							allDay: false,
							</cfif>
						}<cfif record neq last>,</cfif>
					</cfoutput>
